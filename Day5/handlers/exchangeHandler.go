...
func (h *Exchange) ExchangeHandler(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.exchangeService.ConvertAmount(ctx, from, to, amountFloat)
	if err != nil {
		err = fmt.Errorf("handler: failed to convert amount %f from %s to %s: %w", amountFloat, from, to, err)
		logger.Error("ExchangeHandler error", err)
		http.Error(w, "Failed to convert amount", http.StatusInternalServerError)
		return
	}
